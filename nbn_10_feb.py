...
def check_update_data_mongo_adler(request):
    ...
    try:
        ...
        req_data = {field: data.get(field, "") for field in required_fields}

        # If email_body is present, extract fields from it
        if req_data.get("email_body"):
            extracted_fields = read_email_body_adler(req_data["email_body"])
            # Update req_data with extracted fields only if they're empty or None in original data
            for field, value in extracted_fields.items():
                if value and not req_data.get(field):
                    req_data[field] = value

        # Check if the message_id already exists in the MongoDB collection
        existing_doc = collection.find_one({"message_id": message_id})

        if existing_doc:
            missing_fields = [field for field in required_fields if field not in existing_doc or not existing_doc[field]]
            if not missing_fields:
                request["response_from_mongo"] = "Duplicate entry found"
                return request

            update_data = {field: req_data[field] for field in missing_fields if req_data.get(field)}
            ...
            if update_data:
                ...
                request["response_from_mongo"] = "Missing fields updated"
                return request
            else:
                request["response_from_mongo"] = "No new fields to update"
                return request
        else:
            # If no document exists, insert the data
            collection.insert_one(req_data)
            request["response_from_mongo"] = "Data inserted successfully"
            return request
...
def main_api_view_adler(request):
    try:
        ...
        if len(missing_list) != 0:
            ...
            email_body = read_email_body_adler(request)
            request["email_body"] = email_body
            mongo_resp = check_update_data_mongo_adler(req_data)
            if mongo_resp["response_from_mongo"] == "Data inserted successfully":
                print("data inserted")
            elif mongo_resp["response_from_mongo"] == "No data to insert":
                print("No new data")
            else:
